#!/usr/bin/env python3
"""
Test Atlassian MCP Server with SSE Endpoint
Test Jira MCP integration with Atlassian's official MCP server
"""
...
async def test_atlassian_mcp_sse():
    """Test Atlassian MCP server with SSE endpoint."""
    print("🧪 Testing Atlassian MCP Server (SSE)")
    print("=" * 60)
    ...
    # Atlassian MCP server URL with SSE endpoint
    atlassian_mcp_url = "https://mcp.atlassian.com/v1/sse"
    
    print(f"\n🔍 Testing Atlassian MCP: {atlassian_mcp_url}")
    
    try:
        # Configure MCP client for Atlassian MCP with SSE
        servers_config = {
            "jira": {
        ...
        print(f"✅ Atlassian MCP client configured")
        ...
        print(f"❌ Error with Atlassian MCP: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def main():
    """Main test function."""
    print("🧪 Atlassian MCP Server Test (SSE)")
    print("=" * 60)
    
    # Test 1: Atlassian MCP with SSE
    jira_ok = await test_atlassian_mcp_sse()
    ...
    # Final summary
    print(f"\n🎉 Atlassian MCP Test Results:")
    print("=" * 50)
    print(f"✅ Atlassian MCP (SSE): {'PASS' if jira_ok else 'FAIL'}")
