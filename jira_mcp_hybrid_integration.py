#!/usr/bin/env python3
"""
Hybrid Jira MCP Integration using direct Jira API with MCP pattern.
Since the Atlassian MCP server has authentication issues, we'll use the direct API
but follow the same pattern as our successful GitHub MCP integration.
"""
...
class JiraMCPHybridIntegration:
    """Hybrid Jira MCP Integration using direct Jira API with MCP pattern."""
    ...
    async def initialize(self) -> bool:
        """Initialize Jira MCP integration using direct API."""
        try:
            print("🔗 Initializing Jira MCP Integration (Hybrid)...")
        ...
        try:
            if tool_name == "get_projects":
                ...
                return await self._search_issues(
                    arguments.get("jql", "project in projectsWhereUserHasPermission()"),
...
async def main():
    ...
    print("🚀 Jira MCP Integration Demo (Hybrid)")
    print("=" * 50)
    
    # Initialize integration
    jira_mcp = JiraMCPHybridIntegration()
